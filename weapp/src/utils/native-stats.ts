...
function extractWeight(item: BambuHistoryItem): number {
  let totalWeight = 0;
...
function calcPeriodStats(history: BambuHistoryItem[]): PeriodStats {
  ...
  for (const item of history) {
    ...
    if (startTime) { const monthKey = startTime.slice(0, 7); monthly[monthKey] = (monthly[monthKey] ?? 0) + 1; }
    ...
    const nSize = (Array.isArray(item.nozzleInfos) && item.nozzleInfos.length > 0 && item.nozzleInfos[0].diameter)
      ? String(item.nozzleInfos[0].diameter) : (item.nozzleSize ? String(item.nozzleSize) : '0.4');
  ...
  return {
    total_prints: total, successful_prints: success, failed_prints: failed, cancelled_prints: cancelled,
    success_rate: successRate, total_weight_g: Math.round(totalWeightG * 10) / 10, total_duration_hours: totalDurationHours,
    devices, filaments, monthly, duration_distribution: durationDist, failure_stage_distribution: failureStageDist,
    extremes: { longest, shortest, heaviest, lightest, most_colors: mostColors },
    nozzle_size_distribution: nozzleSizeDist, over_500g_count: over500gCount,
    over_500g_rate: total > 0 ? Math.round(over500gCount / total * 1000) / 10 : 0,
    slice_mode_distribution: sliceModeDist, multi_color_count: multiColorCount,
    multi_color_rate: total > 0 ? Math.round(multiColorCount / total * 1000) / 10 : 0,
  };
}

export function calculateNativeStats(records: BambuHistoryItem[]): StatsResult {
  const now = new Date();
  ...
  return {
    stats_lifetime: statsLifetime, stats_7d: stats7d, stats_30d: stats30d,
    activity_heatmap: activityHeatmap, filament_success_stats: filamentSuccessStats,
